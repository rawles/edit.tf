...
var bx = [];	 // is the character at (x,y) in a box?
                 // Boxes are pretty annoying in teletext. By convention,
                 // you need to repeat the control codes in order for the
                 // box to appear. It's safest if we keep track of two
                 // cases. Call a 'safe' box one in which the control codes
                 // have been doubled in this way. Otherwise the box is
                 // 'unsafe'.
...
// How many repeated control codes are needed to make a box.
// This should only ever be 1 or 2.
var minimum_box_level = 2;
...
var export_frame = function() {
	...
	document.getElementById('export').innerHTML =
		...
		+ "<br/> or <a href=\"http://twitter.com/home?status=@edit_tf http://edit.tf/%23"+hashstring+"\" target=\"_blank\">tweet this frame to examples.edit.tf</a>";
		+ "</div>";
	active_export = 1;
}
...
var draw_status_bar_frame = function(ctx) {
	...
	// unboxed or boxed?
	ctx.fillText("B" + bx[cury][curx] + "/" + box_count, offset+(9.85*spacing), 532*pix_scale);
...
var place_code = function(x,y,code,andrender) {
	...
	// repeated in case of errors.
        ...
        // The standards are unclear on how repeated boxed characters should
        // appear. We're going to assume they need to be doubled up.
	if ( code == 10 || code == 11 ) {
		cc[y][x] = code;

		// How many of these codes are in a row?
		var in_a_row = 1; var starting_at = x - 1; 
		while ( starting_at >= 0 && cc[y][starting_at] == code ) { 
			in_a_row++; starting_at--;
		}
		starting_at++;

		console.log(in_a_row + " in a row, starting at " + starting_at);

                // Determine the new value of the 'box level'
		var newbx = bx[y][x];
                if ( code == 10 && in_a_row >= minimum_box_level ) { newbx--; } // Start box.
		if ( code == 11 && in_a_row >= minimum_box_level ) { newbx++; }
                if ( newbx < 0 ) { newbx = 0; } if ( newbx > 1 ) { newbx = 1; }

		// The limit here needs to be after the right number of control codes
		// in a row, since this is the only way the state of bx can change.
		if ( newbx != bx[y][x] ) { 
			var limit = 40;
			var in_a_row_10 = 0;
			var in_a_row_11 = 0;
			if ( minimum_box_level == 1 ) { 
				for ( var c = x + 1; c < 40; c++ ) {
					if ( cc[y][c] == 10 || cc[y][c] == 11 ) {
						limit = c+1; break; // set-after, but the first character.
					}
				}
			}
			if ( minimum_box_level == 2 ) { 
				for ( var c = x + 1; c < 40; c++ ) {
					if ( cc[y][c] == 10 ) { in_a_row_10++; } else { in_a_row_10 = 0; } 
					if ( cc[y][c] == 11 ) { in_a_row_11++; } else { in_a_row_11 = 0; } 

					if ( in_a_row_10 >= 2 && in_a_row_11 >= 2 ) { 
						limit = c; break; // set-after, but the first character.
					}
				}
			}
			for ( var c = starting_at + 1; c < limit; c++ ) { // set-after (the first)
				bx[y][c] = newbx;
			}
			...
			if ( andrender != 0 ) {
				...
				autorender(starting_at, y, limit-starting_at, 1, 1);
			}
		}
	}
}
...
var get_box_count = function() {
	...
	for ( var y = 0; y < 25; y++ ) {
		...
		for ( var x = 0; x < 40; x++ ) {
			if ( cc[y][x] == 10 ) {
				end_in_a_row++;
				if ( end_in_a_row == minimum_box_level
                                        && in_box == 1 ) {
					in_box = 0;
					box_count++;
					}
			...
			if ( cc[y][x] == 11 ) {
				start_in_a_row++;
				if ( start_in_a_row == minimum_box_level
                                        && in_box == 0 ) {
					in_box = 1;
					}
			} else {
...
var check_for_remove_code = function(x, y, andrender) {
	...
	// If we need repeating codes, the the change of state can only
	// happen if the code is part of a group.
	...
	if ( code == 10 || code == 11 ) {
		...
		if ( minimum_box_level == 2 ) { 
			// Determine this cluster, before its deletion.
			// Scan back
			while ( start > 0 && cc[y][start-1] == code ) { start--; }
			newbx = bx[y][start];
			...
			if ( end - start > 0 ) { // multiple codes
				...
				// the next double-code pair.

				var limit = 40; var in_a_row_10 = 0; var in_a_row_11 = 0;
				for ( var c = end + 1; c < 40; c++ ) {
					if ( cc[y][c] == 10 ) { in_a_row_10++; } else { in_a_row_10 = 0; } 
					if ( cc[y][c] == 11 ) { in_a_row_11++; } else { in_a_row_11 = 0; } 
		...
		recompute_box_count = 1;
		if ( andrender != 0 ) {
			autorender(start, y, limit-start, 1);
		}
	}
}
